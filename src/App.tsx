...
function RoleSwitcher({ onLogout }: { onLogout: () => void }) {
  ...
  return (
    <div className="fixed bottom-4 right-4 md:bottom-6 md:right-6 z-[9999] game-panel p-2 flex gap-2 scale-90 sm:scale-100 origin-bottom-right items-center">
      <button
        onClick={onLogout}
        className="px-3 py-2 text-xs font-bold text-gray-500 hover:text-gray-900 mr-2"
      >
        Logout
      </button>
      <button
      ...
      {currentUserMode === "restaurant" && (
        <div className="flex flex-col gap-1 items-end">
          <div className={`flex items-center gap-2 px-4 py-2 text-sm game-btn relative bg-[#EE2737] text-white border-[#EE2737] font-bold shadow-sm`}>
            <Store className="w-4 h-4" />
            <span className="hidden sm:inline game-text">{translate("Restaurant Portal", language)}</span>
            {unreadRestaurant > 0 && (
              <span className="absolute -top-2 -right-2 bg-[#00AA13] text-white text-[10px] w-5 h-5 flex items-center justify-center rounded-full shadow-sm">
                {unreadRestaurant}
              </span>
            )}
          </div>
      ...
      {currentUserMode === "supplier" && (
        <div className="flex flex-col gap-1 items-end">
          <div className={`flex items-center gap-2 px-4 py-2 text-sm game-btn relative bg-[#00AA13] text-white border-[#00AA13] font-bold shadow-sm`}>
...
function Main() {
  ...
  if (!isAuthenticated) {
    return <LandingPage onComplete={() => setIsAuthenticated(true)} />;
  }

  return (
    <>
      <RoleSwitcher onLogout={() => setIsAuthenticated(false)} />
