...
export function SupplierInventory() {
  ...
  const totalSpace = inventory.reduce(
    (sum, item) => sum + (item.spaceUsed || 0),
    0,
  );
  ...
  const handleAdd = (e: React.FormEvent) => {
    ...
    const newItem: any = {
      // Use any temporarily if types are complaining, but SupplierInventoryItem has expectedSupplyDate and supplyCycle. Let's just use it
      id: `sinv-${Date.now()}`,
      name: newName,
      ...
      supplyCycle: newSupplyCycleValue
        ? `${newSupplyCycleValue} ${newSupplyCycleUnit}`
        : "",
    };
  ...
  const handleFileUpload = async (
    ...
    try {
      ...
      if (
        ...
        alert(
          "Sistem AI sedang sibuk karena tingginya permintaan. Silakan coba lagi dalam beberapa saat.",
        );
      } else {
  ...
  const handleAICalculation = async () => {
    ...
    try {
      ...
      const allMenuNames = restaurants.flatMap((r) =>
        r.menu.map((m) => m.name),
      );
      ...
      const prompt = `
        ...
        ${JSON.stringify(
          inventory.map((i) => ({
            name: i.name,
            quantity: i.quantity,
            spaceUsed: i.spaceUsed,
            expirationDate: i.expirationDate,
            expectedSupplyDate: i.expectedSupplyDate,
      ...
      <div className="flex flex-col gap-6">
        <div>
          <div className="bg-white border border-gray-200 p-4 mb-6">
            <h3 className="font-bold text-gray-900 mb-3 text-lg game-text text-[#EE2737]">
              Tambah Barang
            </h3>
            <form onSubmit={handleAdd} className="space-y-3">
              <div>
                <label className="block text-xs  font-bold text-gray-400 mb-1 game-text">
                  Nama Barang
                </label>
                <input
                  type="text"
                  className="w-full bg-white border border-gray-200 text-gray-900 p-2 game-text focus:outline-none focus:border-[#EE2737]"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    type="number"
                    step="0.1"
                    className="w-full bg-white border border-gray-200 text-gray-900 p-2 game-text focus:outline-none"
                    value={newSpace}
                ...
                <div>
                  <div className="flex justify-between items-center mb-1">
                    <label className="block text-xs font-bold text-gray-400 game-text">
                      Tanggal Kedaluwarsa
                    </label>
                    <button
                      type="button"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3">
                <div>
                  <label className="block text-xs font-bold text-gray-400 mb-1 game-text">
                    Tanggal Masuk (Tersedia)
                  </label>
                  <input
                    type="date"
                    className="w-full bg-white border border-gray-200 text-gray-900 p-2 game-text focus:outline-none"
                    value={newExpectedSupplyDate}
                    onChange={(e) => setNewExpectedSupplyDate(e.target.value)}
                  />
                </div>
                <div>
                  <label className="block text-xs font-bold text-gray-400 mb-1 game-text">
                    Siklus Pasokan (Opsional)
                  </label>
                  <div className="flex gap-2">
                    <input
                      type="number"
                      className="w-1/2 bg-white border border-gray-200 text-gray-900 p-2 game-text focus:outline-none"
                      placeholder="Angka"
                      value={newSupplyCycleValue}
                      onChange={(e) => setNewSupplyCycleValue(e.target.value)}
                    />
                    <select
                      className="w-1/2 bg-white border border-gray-200 text-gray-900 p-2 game-text focus:outline-none focus:border-[#EE2737]"
                      value={newSupplyCycleUnit}
                      onChange={(e) => setNewSupplyCycleUnit(e.target.value)}
                    >
              ...
              <button
                type="submit"
                className="w-full py-2 bg-[#EE2737] hover:bg-[#EE2737]/90 text-white font-bold game-text mt-2 flex justify-center items-center gap-2 rounded-sm"
              >
                <Plus className="w-5 h-5" /> Masukkan ke Inventaris
              </button>
            </form>
            ...
            <div className="relative mt-4">
              ...
              <div className="relative flex justify-center text-sm">
                <span className="px-2 bg-white text-gray-400 font-bold game-text   text-xs">
                  ATAU AUTO-IMPORT
                </span>
              </div>
            </div>
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-end mb-2">
              <h3 className="font-bold text-gray-900 text-lg game-text">
                Stok Saat Ini
              </h3>
              <span className="text-gray-400 font-bold game-text text-sm">
                Total Kapasitas: {totalSpace.toFixed(1)} m³
              </span>
            </div>
            {inventory.length === 0 ? (
              <div className="text-center py-6 text-gray-400 font-bold game-text italic">
              ...
              inventory.map((item) => {
                ...
                const percentage =
                  totalSpace > 0
                    ? (((item.spaceUsed || 0) / totalSpace) * 100).toFixed(1)
                    : "0.0";
                const expired = isExpired(item.expirationDate || "");
                return (
                  <div
                    key={item.id}
                    className={`bg-white border p-3 flex justify-between items-center group ${expired ? "border-[#EE2737]/50" : "border-gray-100"}`}
                  >
                    <div className="flex-1 w-full relative">
                      <div className="flex justify-between w-full items-start">
                        <h4 className="font-bold text-xl text-gray-900 game-text flex items-center gap-2">
                          {item.name}
                          {expired && (
                            <span className="bg-[#EE2737] text-white text-[10px] px-2 py-0.5 rounded-sm  ">
                              Expired
                            </span>
                          )}
                        </h4>
                        <button
                          ...
                          className="p-2 text-gray-400 hover:text-[#EE2737] hover:bg-white transition-all rounded z-10 shrink-0"
                        >
                      ...
                      <div className="mt-2 text-xs font-bold text-[#EE2737]  game-text flex flex-col">
                        <span>
                          Est. Market Price: Rp{" "}
                          {dynamicInfo.estimatedPrice.toFixed(2)}
                        </span>
                        <span className="text-[10px] text-gray-400 mb-2">
                          (Demand: {dynamicInfo.marketDemand} | Supply:{" "}
                          {dynamicInfo.marketSupply})
                        </span>
                      </div>

                      {item.expectedSupplyDate && (
                        <div className="mt-1 text-xs font-bold text-[#00AA13] game-text">
                          Tersedia: {item.expectedSupplyDate}{" "}
                          {item.supplyCycle ? `(${item.supplyCycle})` : ""}
                        </div>
                      )}

                      <div className="mt-2 w-full bg-white h-2 rounded overflow-hidden flex border border-gray-100">
                        <div
                          className="bg-[#EE2737] h-full"
                          style={{ width: `${percentage}%` }}
                        ></div>
                      </div>
                      <div className="flex justify-between mt-1 items-center">
                        <div
                          className={`text-xs font-bold game-text ${expired ? "text-[#EE2737]" : "text-[#EE2737]"}`}
                        >
                          Exp: {item.expirationDate || "N/A"}
                        </div>
                        <span className="text-[10px] text-gray-400 font-bold game-text">
                          {percentage}% Kapasitas Dipakai
                        </span>
                      </div>
                    </div>
                  </div>
        ...
        <div className="flex flex-col gap-6">
          <div className="bg-white border border-gray-200 p-6 relative shadow-sm">
            ...
            <p className="game-text text-gray-700 text-sm mb-4 leading-relaxed">
              Peringatan otomatis untuk semua paket (kontaminasi silang,
              pendinginan, dan cara simpan) agar bahan pemasok tetap aman &
              awet.
            </p>
            <button
              ...
              <div className="space-y-3 animate-in fade-in duration-300">
                ...
                {storageWarnings.frozenRequired.length > 0 && (
                  <div className="bg-blue-50 p-3 border border-blue-200 rounded-lg">
                    <strong className="text-xs text-blue-800 block mb-1">
                      ❄️ Wajib Frozen/Chiller
                    </strong>
                    <ul className="text-xs text-blue-700 list-disc pl-4 space-y-1">
                      {storageWarnings.frozenRequired.map((w, i) => (
                        <li key={i}>{w}</li>
                      ))}
                    </ul>
                  </div>
                ...
                {storageWarnings.roomTempRequired.length > 0 && (
                  <div className="bg-orange-50 p-3 border border-orange-200 rounded-lg">
                    <strong className="text-xs text-orange-800 block mb-1">
                      🌡️ Wajib Suhu Ruangan
                    </strong>
                    <ul className="text-xs text-orange-700 list-disc pl-4 space-y-1">
                      {storageWarnings.roomTempRequired.map((w, i) => (
                        <li key={i}>{w}</li>
                      ))}
                    </ul>
                  </div>
                ...
                {storageWarnings.sunDriedRequired.length > 0 && (
                  <div className="bg-yellow-50 p-3 border border-yellow-200 rounded-lg">
                    <strong className="text-xs text-yellow-800 block mb-1">
                      ☀️ Wajib Dijemur/Kering
                    </strong>
                    <ul className="text-xs text-yellow-700 list-disc pl-4 space-y-1">
                      {storageWarnings.sunDriedRequired.map((w, i) => (
                        <li key={i}>{w}</li>
                      ))}
                    </ul>
                  </div>
                  ...
                  storageWarnings.sunDriedRequired.length === 0 && (
                    <div className="bg-green-50 p-3 border border-green-200 rounded-lg text-center">
                      <strong className="text-xs text-green-800">
                        ✅ Penyimpanan saat ini aman dan tidak ada peringatan.
                      </strong>
                    </div>
                  )}
              </div>
          ...
          <div className="bg-white border border-gray-200 p-6 relative shadow-sm">
            <h3 className="font-bold text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-purple-600 text-xl game-title flex items-center gap-2 mb-4">
              ...
              <span className="text-gray-900">Analisa</span> dan evaluasi pintar
            </h3>
            <p className="game-text text-gray-700 text-sm mb-6 leading-relaxed">
              Evaluasi efisiensi gudang, efektivitas pasar terhadap produk
              dominan (seperti Avocado Toast, dsb.), dan peringatan masa
              kedaluwarsa dengan Artificial Intelligence.
            </p>
              ...
              <div className="space-y-3 animate-in fade-in duration-300">
                ...
                <div className="bg-white p-3 border border-gray-100 rounded-lg shadow-sm">
                  <div className="flex items-center gap-2 mb-1.5">
                    <strong className="text-xs text-gray-900 bg-gray-100 px-2 py-0.5 rounded">
                      Status
                    </strong>
                  </div>
                  <p className="text-gray-700 text-xs leading-relaxed">
                    {aiReport.capacityWarning}
                  </p>
                </div>
                ...
                {aiReport.expirationReminders.length > 0 && (
                  <div className="bg-red-50 p-3 border border-red-100 rounded-lg">
                    <div className="flex items-center gap-2 mb-1.5">
                      <strong className="text-xs text-[#EE2737] bg-white px-2 py-0.5 rounded shadow-sm">
                ...
                <div className="bg-white p-3 border border-gray-100 rounded-lg shadow-sm">
                  <div className="flex items-center gap-2 mb-1.5">
                    <strong className="text-xs text-[#00AA13] bg-green-50 px-2 py-0.5 rounded">
                      Sinkronisasi Pasar Dominan
                    </strong>
                  </div>
                  <p className="text-gray-700 text-xs leading-relaxed">
                    {aiReport.dominantProductsAnalysis}
                  </p>
                </div>

                {aiReport.actionableAdvice && (
                  <div className="bg-purple-50 p-3 border border-purple-100 rounded-lg">
                    <p className="text-purple-800 text-xs font-medium italic">
                      "{aiReport.actionableAdvice}"
                    </p>
                  </div>
                )}

                {aiReport.discountRecommendations &&
                  aiReport.discountRecommendations.length > 0 && (
                    <div className="bg-orange-50 border border-orange-200 rounded-lg p-3">
                      <div className="text-[10px] font-bold text-orange-600 mb-2 uppercase tracking-wider flex items-center justify-between">
                        <span>Diskon Promosi / Penawaran</span>{" "}
                        <span className="bg-white px-1.5 py-0.5 rounded shadow-sm text-black">
                          TINGKATKAN PESANAN
                        </span>
                      </div>
                      <div className="space-y-2">
                        {aiReport.discountRecommendations.map((rec, i) => (
                          <div
                            key={i}
                            className="flex justify-between items-center bg-white p-2 rounded shadow-sm border border-orange-100"
                          >
